#
# Use R to run OncoSimX-breast version 3.5.0.90
...
#
# Model digest of OncoSimX-breast version 3.5.0.90: "932ece0e83fdf5764c58844a1840baba"
#
md <- "932ece0e83fdf5764c58844a1840baba"
...
for (scPD in scalePD)
{
  ...
  for (scMA in scaleMA)
  {
    ...
    for (scSC in scaleSC)
    {
      ...
      pd <- list(
          ModelDigest = md,   # model digest of OncoSimX-breast version 3.5.0.90
...
jr <- content(rsp)
taskRunStamp <- jr$RunStamp  # modeling task run stamp

# wait until task completed
runDigests <- waitForTaskCompleted(taskName, taskRunStamp, apiUrl, md)
